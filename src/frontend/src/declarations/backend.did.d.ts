/* eslint-disable */

// @ts-nocheck

// This file was automatically generated by @caffeinelabs/bindgen@0.1.0.
// You should NOT make any changes in this file as it will be overwritten.
// Additionally, you should also exclude this file from your linter and/or formatter to prevent it from being checked or modified.

import type { ActorMethod } from '@icp-sdk/core/agent';
import type { IDL } from '@icp-sdk/core/candid';
import type { Principal } from '@icp-sdk/core/principal';

export interface Balances { 'icpBalance' : bigint, 'ckusdtBalance' : bigint }
export interface _SERVICE {
  'clearLogs' : ActorMethod<[], undefined>,
  'getActiveOrders' : ActorMethod<[], Array<bigint>>,
  'getBalances' : ActorMethod<[], Balances>,
  'getCanisterPrincipal' : ActorMethod<[], string>,
  'getConfig' : ActorMethod<
    [],
    { 'intervalSeconds' : bigint, 'spreadBps' : bigint, 'numOrders' : bigint }
  >,
  'getLogs' : ActorMethod<[], Array<string>>,
  'getStatus' : ActorMethod<
    [],
    {
      'lastMidPrice' : bigint,
      'activeOrderCount' : bigint,
      'isRunning' : boolean,
    }
  >,
  'initCanisterPrincipal' : ActorMethod<[], undefined>,
  'setConfig' : ActorMethod<[bigint, bigint, bigint], undefined>,
  'startBot' : ActorMethod<[], undefined>,
  'stopBot' : ActorMethod<[], undefined>,
  'withdrawCKUSDT' : ActorMethod<[bigint, Principal], undefined>,
  'withdrawICP' : ActorMethod<[bigint, Principal], undefined>,
}
export declare const idlService: IDL.ServiceClass;
export declare const idlInitArgs: IDL.Type[];
export declare const idlFactory: IDL.InterfaceFactory;
export declare const init: (args: { IDL: typeof IDL }) => IDL.Type[];